...
export default class Room extends PIXI.Container {
    ...
    createOneUser(options) {
        ...
        const user = new PIXI.Sprite.from(customProfile || "images/avatar_default.png");
        user.name   = 'player';
        user.width  = 144;
        user.height = 144;
        user.x = (   index === 0
        ...
        let nickname = new PIXI.Text(name, { fontSize: 36, align : 'center', fill: "#515151"});
    ...
    handleRoomInfo(res) {
        ...
        memberList.forEach((member, index) => {
            ...
            if ( databus.owner === member.id ) {
                databus.ownerInfo = member;
                databus.isOwner = true;
                console.log('is owener', member.id)
            ...
            if ( member.isEmpty ) {
                ...
                user.on('pointerdown', () => {
                    console.log(`%c[invite inlink] %c: ${location.protocol}//${location.host}?roomId=${this.gameServer.roomId}`, 'background: green; padding: 0 10px; color: #fff;', '')
                });
            }
        });
