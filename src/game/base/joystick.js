...
export default class JoyStick extends PIXI.Container {
  ...
  onTouchDown(evt) {
    ...
    let limit = this.getPointInCircle(
      this.center,
      this.radius - this.button.radius,
      bx,
      by
    );
  ...
  eventFilter(limit) {
    ...
    if (degree >= low && degree < high) {
      ...
      this.currentDegree =
        parseInt(degree / this.directionDegree) * this.directionDegree;
  ...
  onTouchMove(evt) {
    ...
    if (
      this.currIdentifier !== undefined &&
      evt.data.identifier !== this.currIdentifier
    ) {
      return;
    }
